...
use hyperware_process_lib::{println, LazyLoadBlob, our, hyperapp::source};
...
impl VoiceState {
    ...
    async fn create_call(&mut self, request: CreateCallReq) -> Result<CallInfo, String> {
        ...
        if let Err(e) = hyperware_process_lib::hyperapp::get_server().unwrap().serve_ui(
            "ui-call",
            vec![&call_path],
    ...
    async fn leave_call(&mut self, request: LeaveCallReq) -> Result<(), String> {
        ...
        if should_end_call {
            ...
            if let Err(e) = hyperware_process_lib::hyperapp::get_server().unwrap().unserve_ui("ui-call", vec![&call_path]) {
