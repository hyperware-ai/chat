...
In particular, note the example applications `resources/example-apps/sign/`, `resources/example-apps/id/`, `resources/example-apps/voice`, and `resources/example-apps/file-explorer`.
`sign` and `id` demonstrate local messaging.
`file-explorer` demonstrates VFS interactions.
`voice` demonstrates websockets as well as serving both private and public UIs.
